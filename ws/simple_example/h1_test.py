...
def setupVis(robot : RobotWrapper):
    ...
    # CoM
    robot.viewer['CoM'].set_object(
        meshcat.geometry.Sphere(0.05)
    )
    robot.viewer['CoM_proj'].set_object(
        meshcat.geometry.Sphere(0.05)
    )
    robot.viewer['debug'].set_object(
        meshcat.geometry.Sphere(0.05)
    )
    # DEBUG ankles
    robot.viewer['l_ankle'].set_object(
        meshcat.geometry.Sphere(0.05)
    )
    robot.viewer['r_ankle'].set_object(
        meshcat.geometry.Sphere(0.05)
    )
    robot.viewer['PoS_center'].set_object(
        meshcat.geometry.Sphere(0.05)
    )
    
    return robot

def displayPoint(robot : RobotWrapper,
                 pos : np.ndarray,
                 vis_name : str):
    pos_homo = np.eye(4)
    pos_homo[:3,[3]] = pos
...
def fixJoints(robot : RobotWrapper,
              joint_names : list[str],
              joint_values : list[float] | None = None) -> RobotWrapper:
    ...
    return robot.buildReducedRobot(
        joint_names,
        joint_values)

def proj2PoS(PoS_center : cpin.SE3,
                p : c.SX) -> c.SX:
