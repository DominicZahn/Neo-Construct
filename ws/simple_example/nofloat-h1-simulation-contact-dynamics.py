"""
This is a modified version of the talos example from Pinocchio at ${PINOCCHIO_DIR}/examples/simulation-contact-dynamics.py

It simulates the H1-2 moving towards a random pertubation (q_ref) from the starting configuration (q_0).
For convinience a srdf file is provided. Balancing is done with a simple P-Controller.

The main goal is to showcase the calculation of contact forces using the constraintDynamics functionality.
"""

import math
import sys
import time
from pathlib import Path

import numpy as np
import pinocchio as pin
from pinocchio.visualize import MeshcatVisualizer

# Load the URDF model. (H1-2)
model_path = Path('/home/robot/ws/src/ros2_heinz/h1_gazebo_sim/ros_gz_h1_description/models/')
mesh_dir = model_path.parent.parent
urdf_filename = "h1_2_handless.urdf"
urdf_model_path = model_path / "h1_ign/" / urdf_filename
srdf_filename = "nofloat-h1_2.srdf"
srdf_full_path = srdf_filename

# build custom floating base joint to avoid quaternions in q
root_joint = pin.JointModelComposite()
root_joint.addJoint(pin.JointModelPX())
root_joint.addJoint(pin.JointModelPY())
root_joint.addJoint(pin.JointModelPZ())
root_joint.addJoint(pin.JointModelRX())
root_joint.addJoint(pin.JointModelRY())
root_joint.addJoint(pin.JointModelRZ())


model, collision_model, visual_model = pin.buildModelsFromUrdf(
   urdf_model_path, mesh_dir, root_joint
)

try:
    viz = MeshcatVisualizer(model, collision_model, visual_model)
    viz.initViewer(open=False)
except ImportError as err:
    print(
        "Error while initializing the viewer. "
        "It seems you should install Python meshcat"
    )
    print(err)
    sys.exit(0)

# Load the robot in the viewer.
viz.loadViewerModel()

# Display a robot configuration.
pin.loadReferenceConfigurations(model, srdf_full_path, verbose=True)
q0 = model.referenceConfigurations["knees_bend_0.4"] # knees_bend_0.2 ; knees_bend_0.4 ; T-pose
# ---- Alternative to srdf ----
# ! WARNING : If the quaternion is not a unit quaternion the robot will float over the ground!
# q0 = np.zeros(model.nq)
# q0[:7] = np.array([0., 0., 1., 0., 0., 0., 1.]) # x,y,z, qx,qy,qz,qw
# -----------------------------
q_ref = pin.integrate(model, q0, 0.1 * np.random.rand(model.nv))
viz.display(q0)
input("Open the Meshcat Viewer and than pres RETURN to advance")

feet_name = ["left_ankle_roll_link", "right_ankle_roll_link"]
frame_ids = [model.getFrameId(frame_name) for frame_name in feet_name]

v0 = np.zeros(model.nv)
v_ref = v0.copy()
data_sim = model.createData()
data_control = model.createData()

contact_models = []
contact_datas = []


for frame_id in frame_ids:
    frame = model.frames[frame_id]
    contact_model = pin.RigidConstraintModel(
        pin.ContactType.CONTACT_6D, model, frame.parentJoint, frame.placement
    )
    breakpoint()

    contact_models.append(contact_model)
    contact_datas.append(contact_model.createData())

num_constraints = len(frame_ids)
contact_dim = 6 * num_constraints

pin.initConstraintDynamics(model, data_sim, contact_models, contact_datas)

t = 0
dt = 5e-3

S = np.zeros((model.nv - 6, model.nv))
S.T[6:, :] = np.eye(model.nv - 6)
Kp_posture = 0.9
Kv_posture = 0.05 * math.sqrt(Kp_posture)

q = q0.copy()
v = v0.copy()
tau = np.zeros(model.nv)

T = 15

while t <= T:
    print("t:", np.round(t,3))
    t += dt

    tic = time.time()
    J_constraint = np.zeros((contact_dim, model.nv))
    pin.computeJointJacobians(model, data_control, q)
    constraint_index = 0
    for k in range(num_constraints):
        contact_model = contact_models[k]
        J_constraint[constraint_index : constraint_index + 6, :] = pin.getFrameJacobian(
            model,
            data_control,
            contact_model.joint1_id,
            contact_model.joint1_placement,
            contact_model.reference_frame,
        )
        constraint_index += 6

    A = np.vstack((S, J_constraint))
    b = pin.rnea(model, data_control, q, v, np.zeros(model.nv))

    try: 
        sol = np.linalg.lstsq(A.T, b, rcond=None)[0]
    except:
        sys.exit(1)

    tau = np.concatenate((np.zeros((6)), sol[: model.nv - 6]))

    tau[6:] += (
        -Kp_posture * (pin.difference(model, q_ref, q))[6:]
        - Kv_posture * (v - v_ref)[6:]
    )

    prox_settings = pin.ProximalSettings(1e-12, 1e-12, 10)
    pin.computeJointJacobians(model, data_sim, q)
    data_sim.q_in = q
    for cm, cd in zip(contact_models, contact_datas):
        cm.calc(model, data_sim, cd)
    a = pin.constraintDynamics(
        model, data_sim, q, v, tau, contact_models, contact_datas, prox_settings
    )
    np.set_printoptions(2, suppress=True)
    print("lambda_c L:", data_sim.lambda_c[:6])
    print("lambda_c R:", data_sim.lambda_c[6:])
    np.set_printoptions(suppress=False)
    print("constraint:", np.linalg.norm(J_constraint @ a))
    print("iter:", prox_settings.iter)

    v += a * dt
    q = pin.integrate(model, q, v * dt)

    viz.display(q)
    elapsed_time = time.time() - tic

    time.sleep(max(0, dt - elapsed_time))
    # input()
